...
public class FabricPermissionBridge {
    ...
    @Nullable
    public static String checkPermissionString(CommandSourceStack player, Identifier permission) {
        if (IS_LOADED) {
            var res = player.checkPermission(PermissionNode.ofString(permission));
            if (res != null) {
                return res;
            }
        }
        ...
        if (IS_LOADED_LEGACY) {
            return Options.get(player, permission.toShortLanguageKey().replace('/', '.')).orElse(null);
        }
